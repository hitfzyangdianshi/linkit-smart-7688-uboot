...
{
	char  // * fwoldpath, * fwnewpath, 
		* outfilepath, c,
		hash_old[32], * hash_old_singlechar,
		hash_new[32], * hash_new_singlechar;
	long i;

	if (argc != 4) {
		printf("argc!=4...  \nplease give parameters: old_fw_hashvalue    new_fw_hashvalue    output_fw_info_path\n");
		exit(1);
	}
	else {
		hash_old_singlechar = argv[1];
		hash_new_singlechar = argv[2];
		outfilepath = argv[3];// firmwares/mtd8/mtd8_test2.bin
	}
	...
	for (i = 0; i < 32; i++)hash_new_notfirstboot[i] = hash_new[i];
	for (i = 0; i < 64; i++)hash_new_notfirstboot_singlechar[i] = hash_new_singlechar[i];
	...
	fw_info_test.size_old = 0;
	fw_info_test.size_new = 0;
	...
	memset(publickey_eg1, 0, sizeof(publickey_eg1));
	memset(publickey_eg2, 0, sizeof(publickey_eg2));
	memset(publickey_eg3, 0, sizeof(publickey_eg3));
	memset(publickey_eg4, 0, sizeof(publickey_eg4));
...
/*
hash_old: sizeold (mtd3-mtd6, init-0x357)
hash_new_firstboot: sizenew (init-0x357)
hash_new(not firstboot): sizenew (init-0x357)
*/


// g++ make_mtd8_serverend_withhashes.cpp -o make_fwinfo_s.a.out