...
int make_mtd3(const char initpath[],const char outpath[]) {
	const char* init_filename = initpath;//char init_filename[] = "/tmp/download_fw.bin";  
	long init_file_length = file_size2(init_filename), i;
	FILE* init_file = fopen(init_filename, "rb");
	FILE* output_file = fopen(outpath, "wb");//FILE* output_file = fopen("/tmp/output_file.binmtd3notfirstboot", "wb");
	uint8_t b[16];
	int j;
	bool putFF = false;
...
{
	char *fwoldpath,*fwnewpath, * outfilepath, c,  
		hash_old[32], hash_old_singlechar[64] ,
		hash_new[32], hash_new_singlechar[64];
	long i;

	if (argc != 4) {
		printf("argc!=4...  \nplease give parameters: old_fw new_fw output_fw_info\n");
		exit(1);
	}
	else {
		fwoldpath = argv[1];
	...
	FILE* oldfw, * oldfw_cut, * newfw, * newfw_cut, * newfw_notfirstboot_cut;
	FILE* sha2old, * sha2new, * sha2new_notfirstboot;
